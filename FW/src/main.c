...
static uint8_t IsValidCfgBuf(uint8_t *pCfgBuf);
static UINT GetCfg( uint8_t * pCfgBuf,  cfg_t * pcfg);
static void GetNxtFileName( char * fname);
static void ConvertFileName(uint16_t firstFileName, char * fname);
